...
function PlayerCard({ card }: { card: Card }) {
  ...
  return (
    <div className="ytfut-card-frame" style={wrap}>
      {/* tier background art */}
      <img
        src={t.bg}
        alt=""
      ...
      {card.country && (
        <img
          src={`/badges/flags/${card.country.toLowerCase()}.png`}
          onError={hideOnError}
          alt={card.country}
          style={{
            ...at(17.59, 33.17),
      ...
      {card.categoryLogo && (
        <img
          ...
          crossOrigin="anonymous"
          onError={hideOnError}
          alt={card.categoryLogo.name}
          title={card.categoryLogo.name}
          style={{
            ...at(19.06, 42.25),
            width: "11.875%",
            height: "7.5%",
            objectFit: "contain",
            filter: ink === "#f6f8fb" ? "invert(1)" : "none", // Invert icon for light backgrounds if needed, or keep clean
          }}
        />
      )}
